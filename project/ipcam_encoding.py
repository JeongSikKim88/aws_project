        ...
        for file in files:
            if (file.split(".")[-1].lower() == 'ts'):
                ...
                if os.path.isfile(mp4FilePath):
                    continue
                os.system("./ffmpeg1 -i " + "\"" + filePath + "\" " + "-vcodec copy " + " \"" + mp4FilePath + "\"")
                filePath = os.path.join(root, file)
                print (filePath)
                try:
                    os.remove(filePath)
...
if __name__ == '__main__':
    ...
    pr2 = Process(target=ipcamEncoding(ipcam_ts_folder[1]))
    pr3 = Process(target=ipcamEncoding(ipcam_ts_folder[2]))
    # pr4 = Process(target=ricohEncoding(ricoh_ts_folder[0]))
    # pr5 = Process(target=ricohEncoding(ricoh_ts_folder[1]))
    # pr6 = Process(target=ricohEncoding(heat_ts_folder[0]))
    # pr7 = Process(target=ricohEncoding(heat_ts_folder[1]))
    while true:
        pr1.start()
        pr2.start()
        pr3.start()
